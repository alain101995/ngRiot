...
function ocurrences(runes) {
    ...
    return runes.pages.map(page => {
        ...
        for (let slot of page.slots) { // Iterando en los slots
...
function ocurrences2(runes) {
    ...
    return runes.pages.map(page => {
        ...
        return {
            id: page.id,
            name: page.name,
            runes: page.slots.reduce((acc, slot) => {
                acc[slot.runeId] = (acc[slot.runeId]) ? (acc[slot.runeId] + 1) : 1;
                /* ^
                if (!acc[slot.runeId]) {
                    acc[slot.runeId] = 1;
                }
